...
def send_to_channel(text=None, sticker_id=None):
    """Отправляет сообщение или стикер в ТЕЛЕГРАМ КАНАЛ (только если включен)"""
...
def handle_telegram_command(chat_id, command):
    ...
    if command == '/start':
        welcome_text = (
            ...
            "📢 <b>В канал:</b> Только уведомления о томатах (стикер)\n\n"
            ...
            if 'Ember' in author:
                ...
                if formatted_message:
                    ...
                    if any(tomato in full_text for tomato in ['tomato', 'томат']):
                        ...
                        # Используем популярный стикер с томатом или радостный стикер
                        tomato_sticker_id = "CAACAgIAAxkBAAEL-ohl7HAAAW3dPwABrRcPAAE6AAH2wQACBCYAArVx8UtODdKAAAwKfpo0NAQ"  # Можно заменить на любой другой стикер
