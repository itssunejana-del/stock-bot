...
GARDEN_BOT_USERNAME = "@gardenstockbot"
...
def send_to_garden_bot(message):
    """Отправляет сообщение боту @gardenstockbot"""
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
    data = {
...
def auto_request_stock():
    ...
    while True:
        try:
            # Отправляем запрос боту @gardenstockbot
            success = send_to_garden_bot("🌱 Сток")
            
            if success:
                logger.info("✅ Запрос '🌱 Сток' отправлен")
            else:
...
@app.route('/')
def home():
    return "🤖 Автоматически запрашиваю сток у @gardenstockbot каждую минуту"
...
send_telegram("🔍 Система запущена! Автоматически запрашиваю сток каждую минуту")
